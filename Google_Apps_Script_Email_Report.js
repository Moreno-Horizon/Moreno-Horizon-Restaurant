function doPost(e) {
  try {
    ...
    if (data.command === "sendReport") {
      return handleSendReport(data);
    } else if (data.command === "newBooking") {
...
function handleNewBooking(data) {
  ...
  // Try to find the Master Sheet by name, or create it
  var fileName = "Moreno_Horizon_Master_Bookings";
  ...
  var timestamp = new Date().toLocaleString("en-US", {timeZone: "Africa/Cairo"});
  ...
  sheet.appendRow([
    ...
    b.guests,
    b.restaurant,
    b.engOrderDetails || b.orderDetails || "",
    b.notes || "",
    b.status
  ]);
  ...
  return ContentService.createTextOutput(JSON.stringify({
    "status": "success", 
    "message": "Booking appended to Master Sheet",
    "sheetUrl": ss.getUrl()
  })).setMimeType(ContentService.MimeType.JSON);
}
